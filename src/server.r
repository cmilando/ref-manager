server <- function(input, output, session) {
    ...
    observeEvent(input$backup, {
        cat("> BACKUP DB\n")
        system("cp tmp/lib_df.RDS tmp/lib_df_backup.RDS")
    ...
    df <- reactiveValues()
    df$data <- readRDS("tmp/lib_df.RDS")
    ...
    df_sel <- reactive({
        ...
        saveRDS(select_vars, "tmp/select_var.RDS")
        
        df$data[is.na(df$data)] <- "-"
        
        df_sel <- df$data %>% select(Actions, input$select_var) %>% 
            arrange(year)
    ...
    observeEvent(input$select_button, {
        ...
        selectedRow <- as.numeric(strsplit(input$select_button, 
            "_")[[1]][2])
        ...
        suppressMessages(WriteBib(row_as_bib, "tmp/tmp"))
        this_ref_raw <- readLines("tmp/tmp.bib")
    ...
    observeEvent(input$add_ref_to_lib, {
        ...
        showNotification(paste("Ref add successful"), duration = 3, 
            type = "message")
    })
    ...
    observeEvent(input$edit_ref_in_lib, {
        ...
        df$data <- readRDS("tmp/lib_df.RDS")
        updateTabsetPanel(session, "inTabset", selected = "Table")
        showNotification(paste("Ref edit successful"), duration = 3, 
            type = "message")
    })
    ...
    observeEvent(input$delete_ref_in_lib, {
        ...
        df$data <- readRDS("tmp/lib_df.RDS")
        updateTabsetPanel(session, "inTabset", selected = "Table")
        showNotification(paste("Ref delete successful"), duration = 3, 
            type = "message")
    })
    ...
    output$table_settings = renderRHandsontable({
        req(input$select_var)
        DF <- readRDS("tmp/select_var.RDS")
        
        rhandsontable(DF) %>% hot_table(highlightCol = TRUE, 
            highlightRow = TRUE) %>% hot_cols(colWidths = c(125, 
            100, 100, 100), manualColumnResize = T, columnSorting = T) %>% 
    ...
    observeEvent(input$update_table, {
        ...
        saveRDS(output_table, "tmp/select_var.RDS")
